...
class MyApp(App):
    def build(self):
        ...
        for i in range(self.index_of_upper_cover, len(self.positionen)):
            #widget erstellen und der liste anhängen
            self.widgets.append(AccuratePerspective(image_path = self.image_paths[i]))
            self.widgets[i].change_position(new_x=self.positionen[i][0], new_y=self.positionen[i][1])

            if self.positionen[i][2] == 0:                          #wir sind über dem Mittel album - album muss in den Hinterdrund und nach vorne kippen
                self.widgets[i].change_perspective(richtung= 'vor', new_angle_deg=self.angle_background , new_distance=self.distance_background)
            elif self.positionen[i][2] == 2:                        #wir sind unter dem mittelalbum - album muss in dern Hintergrund und nach hinten kippen
                ...
                self.widgets[i].change_perspective(richtung= 'zurueck', new_angle_deg=self.angle_foreground, new_distance=self.distance_foregorund)

            layout.add_widget(self.widgets[i])


        #Button zum testen hinzufügen
        button_layout = BoxLayout(orientation='horizontal', size_hint_y=0.1)
        btn_1 = Button(text="up")
        btn_1.bind(on_press=lambda x: self.cover_flow_up(self.image_paths, self.widgets, self.positionen, self.index_of_upper_cover))
    ...
    def cover_flow_down(self):
        ...
        self.positionen_zwischen = self.positionen[:]
        for i in range(self.num_widgets- 1, -1, -1):
            #wessen position das widget einnimmt ist in dem positions array definiert - wir scrollen runter, daher muss das widget "hoch" wandern --> index_von_drunter
            #geht das schnellder, wenn man das hard voded?
            self.widgets[i].change_position(
                                    new_x=self.positionen_zwischen[self.positionen[i][3]][0], 
                                    new_y=self.positionen_zwischen[self.positionen[i][3]][1]
                                    )
            
            #self.widgets[i].change_perspective 

        print(self.positionen_zwischen, self.positionen)

        #3. Das unterste widget wird hoch geworfen

        #4. Für das oberste widget wird eine neue textur vergeben


        

    def cover_flow_up(self, image_paths, widgets, positions, index_upper_most_cover):
        pass
