...
def vertices_an_distance_anpassen(resolution, widgetbreite, default_distance, new_distance, vertices):
    #Variablen
    step_in_percent = 1/resolution
    step_in_pixel = 1/resolution * widgetbreite #bildbreite = 200
    widgetbreite_halbiert = round(widgetbreite/2)
...
def forward_punkte_berechnen_(resolution, widgetbreite, db_halbe, dh):
...
def backward_punkte_berechnen_(resolution, widgetbreite, db_halbe, dh):
    #resolution = 16
    step_in_percent = 1/resolution
    step_in_pixel = 1/resolution * widgetbreite #bildbreite = 200
    widgetbreite_halbiert = round(widgetbreite/2)
...
def calc_dh(default_distance, distance, angle_in_dec, widget_seitenlänge):
...
'''
def foward_lean(default_distance, distance, angle_in_dec, widget_seitenlänge, resolution):
    dh = calc_dh(default_distance, distance, angle_in_dec, widget_seitenlänge)
    print(dh)
    indices, vertices_normal, vertices_distant, new_sidelength_diff_by_half = vertices_an_distance_anpassen(resolution, widget_seitenlänge, default_distance, distance)

    new_sidelength_after_distance_change = widget_seitenlänge - 2*new_sidelength_diff_by_half

    indices, vertices_distant, vertices_distand_and_perspective = forward_punkte_berechnen_(resolution, new_sidelength_after_distance_change, dh, dh, vertices_distant)

    return indices, vertices_normal, vertices_distant, vertices_distand_and_perspective
'''

def foward_lean(default_distance, distance, angle_in_dec, widget_seitenlänge, resolution):
    dh = calc_dh(default_distance, distance, angle_in_dec, widget_seitenlänge)

    indices, vertices_normal, vertices_gekippt = forward_punkte_berechnen_(resolution, widget_seitenlänge, dh, dh)
    
    indices, vertices_gekippt, vertices_gekippt_und_distant, new_sidelength_diff_by_half = vertices_an_distance_anpassen(resolution, widget_seitenlänge, default_distance, distance, vertices_gekippt)

    return indices, vertices_normal, vertices_gekippt, vertices_gekippt_und_distant

def backward_lean(default_distance, distance, angle_in_dec, widget_seitenlänge, resolution):
    dh = calc_dh(default_distance, distance, angle_in_dec, widget_seitenlänge)

    indices, vertices_normal, vertices_gekippt = backward_punkte_berechnen_(resolution, widget_seitenlänge, dh, dh)
    
    indices, vertices_gekippt, vertices_gekippt_und_distant, new_sidelength_diff_by_half = vertices_an_distance_anpassen(resolution, widget_seitenlänge, default_distance, distance, vertices_gekippt)

    return indices, vertices_normal, vertices_gekippt, vertices_gekippt_und_distant

def plot_vertices(vertice_1, vertice_2, vertice_3):
    ...
    plt.scatter(x3, y3, color='grey', label='Vertices 3', s=20)
...
i, v1, v2, v3= backward_lean(100, 120, 20, 200, 4)

#v2 = []
#v3 = []

plot_vertices(v1, v2, v3)

#der entscheidende Part ist, dass nun abhängig von der angestrebten Dauer und FPS der aniation zwischenschritte für jeden Punkt berechnet werden können
duration_in_sec = 1
frames_per_sec  = 60

def calc_zwischenpunkte(duration_in_sec, frames_per_sec, vertices_begin, vertices_end):
    zwischenpunkte = []
    #startpunkt sind die vertices_begin
    total_num_frames = duration_in_sec * frames_per_sec
